...
const GameCard: React.FC<GameCardProps> = ({ game }) => {
    ...
    const homeInjuries = market_context?.injuries?.home || [];
    const awayInjuries = market_context?.injuries?.away || [];
    ...
    const isLive = game.status && !game.status.includes('Final') && !game.status.includes('Scheduled');
    ...
    return (
        <motion.div
            ...
            whileHover={{ y: -4 }}
            className="glass rounded-xl p-0 cursor-pointer relative overflow-hidden group border border-white/5 bg-surface/40 hover:bg-surface/60 transition-all shadow-lg"
        >
            {/* Header / Status Bar */}
            <div className="px-5 py-3 border-b border-white/5 flex justify-between items-center bg-black/20">
                <div className="flex items-center gap-2">
                    <Badge variant="outline" className="text-[10px] font-bold border-white/10 bg-white/5 text-white">
